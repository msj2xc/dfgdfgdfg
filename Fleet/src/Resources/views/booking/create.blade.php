...
{{ Form::open(['url' => 'booking', 'method' => 'post','class'=>'needs-validation','novalidate']) }}
<div class="modal-body">
    <div class="row">
        @if (\Auth::user()->type != 'client')
            <div class="form-group col-6">
                {{ Form::label('customer_name', __('Customer Name'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::select('customer_name', $customer, '', ['class' => 'form-control', 'required' => 'required']) }}
            </div>
        @endif
        <div class="col-md-6">
        ...
        <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control" id="ship-address" name="start_address" placeholder="Enter Start Location" required
                autocomplete="off">
        </div>
        <div class="form-group col-md-6">
            {!! Form::label('end_address', __('End Location'), ['class' => 'form-label']) !!}<x-required></x-required>
            <input type="text" class="form-control" id="ship-addresses" name="end_address" placeholder="Enter End Location" required
                autocomplete="off">
        </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                {{ Form::select('trip_type', ['Single Trip' => 'Single Trip', 'Round Trip' => 'Round Trip'], null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Select Trip Type']) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('total_price', __('Total Price'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::number('total_price', null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Enter Total Price']) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('status', __('Status'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::select('status', ['Yet to start' => 'Yet to start', 'Completed' => 'Completed', 'OnGoing' => 'OnGoing', 'Cancelled' => 'Cancelled'], null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Select Trip Status']) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('notes', __('Notes'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::text('notes', null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Enter Notes']) }}
            </div>
        </div>
    </div>
</div>

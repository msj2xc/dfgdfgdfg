...
@section('page-action')
    <div class="d-flex">
        @stack('addButtonHook')
        <a href="{{ route('driver.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
                    ...
                    <div class="card-header border border-bottom h-100 p-3">
                        <div class="user-img-wrp d-flex align-items-center">
                            <div class="user-image rounded border-2 border border-primary">
                                <a href="{{ check_file($driver->avatar) ? get_file($driver->avatar) : get_file('uploads/users-avatar/avatar.png') }}"
                                    target="_blank">
                                    <img src="{{ check_file($driver->avatar) ? get_file($driver->avatar) : get_file('uploads/users-avatar/avatar.png') }}"
                                        alt="user-image" class=" h-100 w-100">
                                </a>
                            </div>
                            <div class="user-content">
                                <h4 class="mb-2 text-break">{{ $driver_name->name ?? ($driver->name ?? '-') }}</h4>
                                <span class="text-dark text-md">{{ $driver_name->email ?? ($driver->email ?? '-') }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="card-body text-center p-3">
                        <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @if (Laratrust::hasPermission('driver edit') ||  Laratrust::hasPermission('driver show') || Laratrust::hasPermission('driver delete'))
                                @if (!empty($user->id))
                                    ...
                                    @permission('driver edit')
                                        <a href="#!" data-url="{{ route('driver.edit', $driver->user_id) }}"
                                            data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Driver') }}"
                                    ...
                                    @permission('driver delete')
                                        ...
                                        <a href="#!" aria-label="Delete" data-confirm="{{ __('Are You Sure?') }}"
                                            ...
                                            data-confirm-yes="delete-form-{{ $driver->id }}"
        ...
        @permission('driver create')
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <a href="#" class="btn-addnew-project border-primary p-4" data-ajax-popup="true" data-size="lg"
                    data-title="{{ __('Create Driver') }}" data-url="{{ route('driver.create') }}">
                    <div class="bg-primary proj-add-icon">
                        <i class="ti ti-plus my-2"></i>
                    </div>
                    <h6 class="my-2 text-center">{{ __('New Driver') }}</h6>
                    <p class="text-muted text-center mb-0">{{ __('Click here to Create New Driver') }}</p>
                </a>
            </div>
        @endpermission
    </div>
    {!! $drivers->links('vendor.pagination.global-pagination') !!}
@endsection

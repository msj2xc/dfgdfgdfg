                ...
                <div class="card-body table-border-style">
                    <div class="table-responsive">
                        <table class="table datatable pc-dt-simple" id="test">
                            ...
                            @php
                                $commissionAgent = App\Models\User::where('id',$bank_transfer_payment->user_id)->pluck('name')->first();
                            @endphp
                                <tr>
                                    <td>{{$bank_transfer_payment->order_id}}</td>
                                    <td>{{ company_datetime_formate($bank_transfer_payment->created_at)}}</td>
                                    <td>{{ !empty($commissionAgent) ? $commissionAgent : '' }}</td>
                                        ...
                                        <span>
                                            <div class="action-btn bg-primary ms-2">
                                                <a  class="mx-3 btn btn-sm  align-items-center"
                                                    data-url="{{ route('commission-bank-transfer.edit', $bank_transfer_payment->id) }}"
                                            ...
                                            <div class="action-btn bg-danger ms-2">
                                                {{Form::open(array('route'=>array('commission-bank-transfer.destroy', $bank_transfer_payment->id),'class' => 'm-0'))}}
                                                @method('DELETE')
                                                    <a
